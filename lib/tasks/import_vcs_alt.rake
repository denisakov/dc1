namespace :import do
	desc "imports data from VCS website"
	task :vcs_alt => :environment do
	require 'rubygems'
	require 'nokogiri'
	require 'open-uri'
	require 'mechanize'

agent = Mechanize.new
agent.idle_timeout = 0.9
#Load the page into Mechanize
page = agent.get("https://vcsprojectdatabase2.apx.com/myModule/Interactive.asp?Tab=Projects&a=1")
#Define maximum number of pages
#Find the link with javascript, which shows the number of pages
m = page.link_with(:text => "move last").attributes['href'].to_s
#Parse the string from javascript to see the maximum number of pages
max = m[35..m.index(')')-11].to_i
#Set the counter for the pages
i = 1
#Start the loop for the pages
max.times do
	#[DOESN'T WORK]Send the POST request and pass the number of a page as a parameter
	agent.post("https://vcsprojectdatabase2.apx.com/myModule/Interactive.asp", "Tab" => 'Projects', "X999whichpage" => i)
	#Pass the page from Mechanize to Nokogiri, now it will be a String of elements
	page = agent.page.parser()
	#This is the number of the first Project title in the table, will be used to find the number of rows
	a = 13
	#Loop through all elements which are not nil
	while !page.css("td")[a].nil? do
		a += 7
	end
	#This is the number of rows which contain the titles
	b = (a-13)/7-1
	#This is the number of the first project title, again
	c = 13
	#Loop to retrieve the data and put it in the database
	b.times do
		title = page.css("td")[c].text.to_s.encode("UTF-16BE", :invalid => :replace, :replace => "'").encode("UTF-8").gsub("\u0092", "'")
		country_name = page.css("td")[c+2].text.to_s
		project = Project.create!(:title => title)
		project.countries.build(:name => country_name)
		project.standards.build(:name => 'VCS')
		project.save!
		c += 7
	end
	i += 1
end
end
end